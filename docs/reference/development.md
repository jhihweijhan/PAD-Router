...
`pyproject.toml` 宣告 Python `>=3.10` 與固定版本 `pywebview==5.4`；本專案以 `uv` 作為唯一執行入口。完整盤面入口 `--gui` 保持既有 Tk 流程，直到 cutover #14；Issue #9–#11 capture/review/rules/search workspace 僅由明確 `--webview` flag 啟動，固定使用 GTK backend。
...
- `test_pad_router_gui.py`：PNG 解析、校正、GUI controller、人工修正、原型資料原子持久化、辨識／中心圖樣、來源顯示縮放、規則自動套用、問號重試、執行前學習、webview bridge 快照／非同步擷取、5×6 review DTO、rules/search invalidation、stale-generation rejection、cancelled previews 與 entrypoint routing。
