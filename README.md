...
規則設定不需要手填 JSON：從三個「消珠條件」選單挑選最多三項，再選擇「全部符合」或「任一符合」。內建選項包括 Combo 門檻、指定屬性、同時消除多屬性、同屬性多組、色珠一橫列、9 顆正方形、十字型、4 顆消除、L 型、T 型、同色連珠與強化珠；危害珠策略、外部條件、搜尋嘗試次數與隨機種子也都是固定選項。
