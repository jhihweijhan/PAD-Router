...
規則設定不需要手填 JSON：從三個「消珠條件」選單挑選最多三項，再選擇「全部符合」或「任一符合」。每個條件旁都有色珠選單；色珠一橫列、9 顆正方形、十字型、4 顆消除、L 型與 T 型都會限定為選取的顏色，其他珠子仍按一般三珠以上消除並計入 Combo。危害珠策略、外部條件、搜尋嘗試次數（5 至 50，每次加 5）與隨機種子也都是固定選項。
