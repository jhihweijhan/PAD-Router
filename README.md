...
規則設定不需要手填 JSON：三個「消珠條件」預設都是「不限」，代表最大 Combo 模式，不會指定色珠。選擇色珠一橫列、9 顆正方形、十字型、4 顆消除、L 型或 T 型後，旁邊的色珠選單才會啟用，並限定該形狀的顏色；一橫列只要有一條完整的同色 6 珠橫列即滿足，即使同時連到額外的同色珠也有效。其他珠子仍按一般三珠以上消除並計入 Combo。消珠結算可選擇「計入落珠連鎖」或「只計轉珠直接消除」；後者不會把重力落下後的新 Match 計入 Combo 或隊長技。危害珠策略、外部條件、搜尋嘗試次數（5 至 50，每次加 5）、執行步數上限（預設 50，可選 30 至 100、每次加 5）與隨機種子也都是固定選項。
